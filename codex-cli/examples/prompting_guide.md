...
```
open-codex "write 2-3 sentences on what you can do"
```

To get a feel for the mechanics, let's ask OpenCodex to create a simple HTML webpage. In a new directory run:

```
mkdir first-task && cd first-task
git init
open-codex "Create a file poem.html that renders a poem about the nature of intelligence and programming by you, OpenCodex. Add some nice CSS and make it look like it's framed on a wall"
```

By default, OpenCodex will be in `suggest` mode. Select "Yes (y)" until it completes the task.
