      ...
      {parts.map((part, i) => {
        ...
        if (content.length > tailSize && frameCount % 2 === 0) {
           ...
           return (
             <Text key={i}>
               {head}
               <Text color={theme.highlight} bold>{tail}</Text>
             </Text>
           );
        }
...
export const TerminalChatResponseToolCallOutput = React.memo(function TerminalChatResponseToolCallOutput({
  content,
  fullStdout,
  toolCall,
  theme,
...
export const TerminalChatResponseToolCall = React.memo(function TerminalChatResponseToolCall({
  message,
  loading = false,
  theme,
}: {
...
export const TerminalChatResponseMessage = React.memo(function TerminalChatResponseMessage({
  ...
  const thoughtRegex = /<(thought|think|thinking)>([\s\S]*?)(?:<\/(?:thought|think|thinking)>|$)/gim;
  ...
  displayContent = displayContent
    .replace(/<\/(thought|think|thinking|plan|roadmap|response)>| <\/(thought|think|thinking|plan|roadmap|response)>/gim, "")
    .trim();
