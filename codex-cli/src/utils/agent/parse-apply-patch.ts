...
export function parseApplyPatch(patch: unknown): ParseResult {
  ...
  if (!normalizedPatch.startsWith(PATCH_PREFIX)) {
    ...
    if (
      normalizedPatch.includes("*** Update File:") ||
  ...
  if (
    normalizedPatch.startsWith(PATCH_PREFIX) &&
    normalizedPatch.endsWith(PATCH_SUFFIX)
  ) {
...
export function parseApplyPatchLegacy(
  patch: unknown,
): Array<ApplyPatchOp> | null {
