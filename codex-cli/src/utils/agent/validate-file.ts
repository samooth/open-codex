...
function getLinterCommand(filePath: string): string | null {
  ...
  if (ext === ".py") {
    if (existsSync(join(cwd, "ruff.toml")) || existsSync(join(cwd, ".ruff.toml")) || existsSync(join(cwd, "pyproject.toml"))) {
...
export async function validateFileSyntax(filePath: string): Promise<ValidationResult> {
  ...
  try {
    switch (ext) {
      ...
      case ".tsx":
        // Fast tsc check
        try {
          execSync(`npx tsc --noEmit --target esnext --skipLibCheck "${filePath}"`, { stdio: "ignore", timeout: 10000 });
        } catch { /* ignore tsc missing */ }
        break;
      case ".py":
        try {
          ...
          execSync(`python3 -c "compile(${escapedContent}, '${filePath}', 'exec')"`, { stdio: "pipe", timeout: 5000 });
        } catch (pyErr: any) {
           return {
            isValid: false,
  ...
  // Only runs if basic syntax passed.
