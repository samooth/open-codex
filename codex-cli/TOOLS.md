...
- **Routing**: The `handleFunctionCall()` method receives the model's request and routes it to the appropriate handler.
...
- **Direct Handlers**: It implements simple file system tools directly (`handleReadFile`, `handleWriteFile`, `handleDeleteFile`, `handleListDirectory`).
...
- **Schema Validation**: Uses `zod` to validate tool arguments.
...
- **Leniency**: Controls how strictly the agent requires `+` prefixes or hunk headers. Recently updated to be more lenient with model hallucinations during file creation.
...
- **`~/.codex/config.json`**: Controls the default `approvalMode` and API providers.
