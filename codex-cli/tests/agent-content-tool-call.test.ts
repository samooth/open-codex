...
describe("AgentLoop Content Tool Call Fallback", () => {
  ...
  it("extracts and executes multiple tool calls from message content", async () => {
    ...
    expect(
      JSON.parse(toolCallMessage![0].tool_calls![0].function.arguments).cmd,
    ).toEqual(["ls"]);
    expect(
      JSON.parse(toolCallMessage![0].tool_calls![1].function.arguments).cmd,
    ).toEqual(["cat", "README.md"]);
  });
  ...
  it("extracts tool calls from a string-based command", async () => {
          ...
          choices: [
            {
              delta: {
                role: "assistant",
                content:
                  'I will read the file now: {"command": "cat README.md"}',
              },
              finish_reason: "stop",
            },
          ],
    ...
    const args = JSON.parse(
      toolCallMessage![0].tool_calls![0].function.arguments,
    );
