...
describe("Interactive Interaction Detection", () => {
  describe("Yes/No Detection", () => {
    it("detects basic confirmation questions", () => {
      expect(detectInteraction("Should I proceed?")).toEqual({
        type: "yes-no",
      });
    ...
    it("detects explicit (yes/no) markers", () => {
      expect(detectInteraction("Ready to apply changes (yes/no)")).toEqual({
        type: "yes-no",
      });
    });

    it("detects complex phrasing with question marks", () => {
      expect(detectInteraction("Would you like me to fix the bug?")).toEqual({
        type: "yes-no",
      });
      expect(detectInteraction("Shall I delete the file?")).toEqual({
        type: "yes-no",
      });
    });
    ...
    it("returns null for non-confirmation questions", () => {
      ...
      expect(
        detectInteraction("What aspect would you like me to dive deeper on?"),
      ).toBeNull();
      ...
      expect(
        detectInteraction("### How would you like to proceed?"),
      ).toBeNull();
      expect(
        detectInteraction("Can I help you with anything else?"),
      ).toBeNull();
      expect(
        detectInteraction("I've finished. Can I do something else?"),
      ).toBeNull();
      expect(detectInteraction("How?")).toBeNull();
      expect(detectInteraction("What should I do next?")).toBeNull();
      expect(
        detectInteraction("The task is done. How should I proceed?"),
      ).toBeNull();
      expect(
        detectInteraction(
          "Hi! How can I assist you today? Feel free to ask any questions about Bitcoin SV, blockchain technology, or any other topics.",
        ),
      ).toBeNull();
    });
  });
  ...
  describe("Multi-choice Detection", () => {
    it("ignores standard Markdown links", () => {
      const msg =
        "Please refer to [Installation Guide](docs/install.md) or [User Guide](docs/user.md).";
    ...
    it("detects choices when explicitly asked to select", () => {
      const msg =
        "Please select one: [Retry] [Ignore] [Abort]. This will affect the build.";
      expect(detectInteraction(msg)).toEqual({
        type: "choices",
    ...
    it("filters out long or redundant entries", () => {
      const msg =
        "Here are the results [Small] [This is a very long string that should probably not be a button in the TUI].";
    ...
    it("should NOT trigger for a list of items that are not choices", () => {
      const msg =
        "I found these files: [file1.ts], [file2.ts], and [file3.ts].";
      expect(detectInteraction(msg)).toBeNull();
    });

    it("should NOT trigger for a list followed by an unrelated question", () => {
      const msg =
        "I found these files: [file1.ts], [file2.ts]. How can I help you today?";
      expect(detectInteraction(msg)).toBeNull();
    });
  });
});
