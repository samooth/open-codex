...
describe("Autocomplete Logic", () => {
  describe("getFileSearchMatch", () => {
    it("detects trigger at start of string", () => {
      expect(getFileSearchMatch("@file")).toEqual({ query: "file", startIndex: 0 });
    });

    it("detects trigger after space", () => {
      expect(getFileSearchMatch("check @src")).toEqual({ query: "src", startIndex: 6 });
    });

    it("detects trigger inside brackets", () => {
      expect(getFileSearchMatch("(@foo")).toEqual({ query: "foo", startIndex: 1 });
      expect(getFileSearchMatch("[@bar")).toEqual({ query: "bar", startIndex: 1 });
    });

    it("detects trigger inside quotes", () => {
      expect(getFileSearchMatch('"@baz')).toEqual({ query: "baz", startIndex: 1 });
      expect(getFileSearchMatch("'@qux")).toEqual({ query: "qux", startIndex: 1 });
    });
